...
export const Navbar: React.FC<NavbarProps> = ({ onLoginClick, onReset, hasImages = false }) => {
  ...
  const handleLogoClick = (e: React.MouseEvent) => {
    e.preventDefault();
    // NUCLEAR OPTION: Hard browser reload to guarantee state clear
    if (hasImages) {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-20 flex items-center px-4 md:px-8 z-30 pointer-events-none">
      ...
      {/* Logo Area */}
      <div className="bg-white/80 backdrop-blur-md px-5 py-2.5 rounded-2xl shadow-card border border-white flex items-center gap-4 pointer-events-auto group mr-4">
      ...
      <div className="ml-auto pointer-events-auto flex items-center gap-4">
        ...
        {!user ? (
          ...
          <div className="bg-white/80 backdrop-blur-md pl-1.5 pr-4 py-1.5 rounded-2xl shadow-card border border-white flex items-center gap-3">

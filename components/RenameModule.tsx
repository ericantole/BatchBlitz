    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center">
            <label className="text-xs text-ink-main font-medium">Enable Rename</label>
        ...
        <button 
          ...
          className={`w-12 h-7 rounded-full relative transition-colors duration-200 ease-in-out focus:outline-none ${rename.enabled ? 'bg-apple-green' : 'bg-gray-200'}`}
        >
          <span 
            className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full shadow-sm transform transition-transform duration-200 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${rename.enabled ? 'translate-x-5' : 'translate-x-0'}`} 
          />
        </button>
      </div>

      {/* Pattern Input */}
      <div className={`space-y-3 transition-all duration-300 ${rename.enabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
